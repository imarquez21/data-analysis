...
lines = f.readlines()
with open(date+'_capacity.csv', 'wb') as csvfile:
  ...
  for l in lines:
    ...
    cur.execute("select deviceid,srcip,dstip,eventstamp,average,std,minimum,maximum,median,direction from m_capacity where deviceid = %s",[device_mac])
    out1 = cur.fetchone()
